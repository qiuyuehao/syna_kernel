...
static int bbk_xxx_fw_update(const struct firmware *fw)
{
	...
	retval = reflash_wrapper_run_fw_update(fw->data,
					(unsigned int)fw->size);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
			"failed to do fw update\n");
...
static int bbk_xxx_mode_change(int which)
{
	...
	case 0:
		if (intf_hcd->is_lpwg) {
			retval = intf_helper_set_dynamic_config(
					DC_IN_WAKEUP_GESTURE_MODE, 0);
			if (retval < 0) {
				LOGE(tcm_hcd->pdev->dev.parent,
					"fail to disable lpwg mode\n");
				goto exit;
			}
			intf_hcd->is_lpwg = false;
		}

		retval = intf_helper_set_sleep(false);
		break;
	case 1:
		if (intf_hcd->is_lpwg) {
			retval = intf_helper_set_dynamic_config(
		...
		retval = intf_helper_set_sleep(true);
		break;
	case 2:
		retval = intf_helper_set_dynamic_config(
					DC_IN_WAKEUP_GESTURE_MODE, 1);
		intf_hcd->is_lpwg = true;
		break;
	default:
		LOGE(tcm_hcd->pdev->dev.parent,
			"unknown selection (%d)\n", which);
