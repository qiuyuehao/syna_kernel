...
int zeroflash_get_fw_image(void)
{
	int retval;
	struct syna_tcm_hcd *tcm_hcd = zeroflash_hcd->tcm_hcd;
...
int zeroflash_download_app_firmware(void)
{
	int retval;
...
{
	int retval;
	unsigned char status_report[10];
...
int zeroflash_download(void)
{
	int retval;
	char retry = 5;
	
	retval = zeroflash_download_app_firmware();
	...
	udelay(1000);
exit:
//	touch_init(zeroflash_hcd->tcm_hcd);
	return retval;
}
