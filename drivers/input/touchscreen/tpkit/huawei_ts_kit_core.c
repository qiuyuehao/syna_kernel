...
static struct dsm_dev dsm_tp = {
	.name = "dsm_tp",
	.device_name = "TP",
	.ic_name = "synaptics",	/*just for testing, waiting for the module owner revised*/
	.module_name = "NNN",
	.fops = NULL,
	.buff_size = 1024,
};
...
int ts_i2c_read(u8* reg_addr, u16 reg_len, u8* buf, u32 len);
int ts_spi_read(u8* reg_addr, u16 reg_len, u8* buf, u32 len);
...
int ts_spi_write(u8* buf, u16 length)
{
	...
	return spi_sync(g_ts_kit_platform_data.spi, &m);
}
...
//TODO: Temp function for DB810 used
static int ts_creat_spi_client(void)
{
	struct spi_master* adapter = NULL;
    struct spi_device* client = NULL;
...
int ts_kit_put_one_cmd(struct ts_cmd_node* cmd, int timeout)
{
    ...
    smp_mb();
    spin_unlock_irqrestore(&q->spin_lock, flags);
    TS_LOG_INFO("put one cmd :%d in ring buff\n", cmd->command);
    error = NO_ERR;
...
int huawei_ts_chip_register(struct ts_kit_device_data* chipdata)
{
    ...
    {    
        if (chipdata->ops->chip_detect) //多余？
        {
