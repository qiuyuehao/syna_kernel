...
struct ts_glove_info
{
    ...
    int op_action;
    int status;
    int glove_switch_addr;
    u16 glove_switch_bit;
};

struct ts_holster_info
{
    int holster_supported;
    u8 holster_switch;
    int op_action;
    int status;
    int holster_switch_addr;
...
struct ts_kit_device_data
{
	int is_parade_solution;
	int is_direct_proc_cmd;
	int support_s3320_short_test;
    int is_i2c_one_byte;
    int is_new_oem_structure;
    int disable_reset;
    int is_in_cell;
    int report_tui_enable;
    u8 tui_set_flag;
	u8 provide_panel_id_support;
    int need_wd_check_status;
...
struct ts_bus_info
{
    ...
    int (*bus_read) (u8* reg_addr, u16 reg_len, u8* buf, u32 len);
};
